...
export default async function JourneysPage({ searchParams }: PageProps) {
  ...
  let res: { insights: Journey[]; total: number } | null = null;
  let fetchError = false;
  try { res = await fetchJourneys(token, PAGE_SIZE, offset); } catch { fetchError = true; }
  const journeys = res?.insights ?? [];
  const total    = res?.total ?? 0;
  const hasPrev  = page > 1;
  const hasNext  = offset + journeys.length < total;
    ...
    <div className="p-6 space-y-6 max-w-4xl page-enter">
      ...
      {fetchError ? (
        <div className="empty-state">
          ...
          <p className="text-sm font-medium mb-1" style={{ color: '#475569' }}>Could not load journey data</p>
          ...
          {(hasPrev || hasNext) && (
            <div className="flex items-center justify-between pt-2">
              ...
              <span className="text-xs tabular-nums" style={{ color: '#475569' }}>
                {offset + 1}–{Math.min(offset + journeys.length, total)} of {total}
              </span>
              {hasNext ? (
                <Link
                  href={`/journeys?page=${page + 1}`}
        ...
        <div className="empty-state">
          ...
          <TriggerAnalysisButton agent="journey" />
        </div>
      )}
    </div>
  );
