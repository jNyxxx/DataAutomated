...
| AUD-07 | **SSE detection via 5s DB poll (ADR-012 note):** resolved by moving to `realtime_events` EventBroker with PostgreSQL `LISTEN/NOTIFY`. | DB load grows with open dashboards. | **Resolved** for MVP via app-level EventBroker. | P8 |
...
| RISK-13 | SSE connection load from 5s server-side poll (AUD-07) | Scale | Med | Low | **Low** | Resolved via Event Broker and LISTEN/NOTIFY | P8 |
